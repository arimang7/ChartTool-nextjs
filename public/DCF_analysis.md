...
■ Pure Narrative Premium (시장 심리, 테마 모멘텀, 멀티플 확장 기대)은  
내재가치 계산에 직접 반영하지 않는다.

다만 아래 5가지 구조적 프리미엄은  
Execution Value 또는 Ecosystem Value에  
귀속 가능한 경우 반영할 수 있다.
...
구조적 프리미엄은 독립 항목으로  
가산하지 않는다.  
해당 프리미엄이 귀속되는 Execution 또는  
Ecosystem 마일스톤의  
아래 3가지 파라미터 중 하나 이상을 조정하는  
방식으로만 반영한다.
...
7. 한국어 + 재무용어 영문 병기
8. “추가 분석 가능 항목” 2~3개
9. DCF 모델로 현재가가 설명되지 않을 경우 구조적 프리미엄에 반영
