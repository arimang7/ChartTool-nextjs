...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      const rssPromise = fetch(rssUrl).then(res => res.text());
      const timeoutPromise = new Promise<string>((_, reject) => setTimeout(() => reject(new Error("News fetch timeout")), 2000));
      const text: string = await Promise.race([rssPromise, timeoutPromise]) as string;
      ...
      while ((match = itemRegex.exec(text)) !== null && count < 6) {
        ...
        if (decodedTitle && link) {
          newsList.push({ title: decodedTitle, link, publisher: "Yahoo Finance" });
          count++;
        }
      }
    } catch (e) {
